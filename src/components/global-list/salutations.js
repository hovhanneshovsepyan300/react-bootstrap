...
export default class Salutations extends Component {
    ...
    render(){
        ...
        const companiesMap = companies.map(({id,company}) =>{
            return (
                <ListMap key={`${id}-${company}`} name={company} id={id} placeholder="Salutation" save={this.save} deleteItem={this.deleteItem} />
            )
        })
        ...
        return (
            <div className="col col-12 col-sm-12 col-md-6 col_left">
                ...
                <div className={`box_form ${companies.length > 10? 'scrolleffect':''}`}>
                    {addItem && <ListMap   placeholder="Salutation" save={this.adding} deleteItem={this.cancelAdding} />}
                ...
                <Modal show={showModal}  onHide={this.handleShowModal} centered>
                    ...
                    <Modal.Body>salutations with name {lastDeletedItem} successfuly deleted</Modal.Body>
